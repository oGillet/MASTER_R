...
transports <- osmdata::osmdata_sf(database)
transports <- transports$osm_points

map <- leaflet() %>%
  setView(lng = longitude[1], lat = latitude[1], zoom = 14) %>% 
  addProviderTiles("Stamen.Toner") %>% 
  # addMarkers(data = transports,
  #            icon = icons(iconUrl="https://cdn-icons-png.flaticon.com/512/2830/2830211.png",
  #                         iconWidth = 25, 
  #                         iconHeight = 25),
  #            group = "Station de bus") %>%
  addCircles(data = transports,
             color = "blue",
             fill = "blue",
             opacity = 1, 
             group = "Station de bus") %>% 
  ...
  addLegendCustom(colors = c("red","blue"), labels = c("Ma position", "Station de bus"), sizes = c(20,20))
...
df_sf_buffer_5000 <- st_buffer(df_sf %>% st_transform(2154),5000) %>% st_transform(4326)

map <- leaflet() %>%
  fitBounds(min(st_coordinates(df_sf_buffer_5000)[,1]),
            min(st_coordinates(df_sf_buffer_5000)[,2]),
            max(st_coordinates(df_sf_buffer_5000)[,1]),
            max(st_coordinates(df_sf_buffer_5000)[,2]))%>% 
  addProviderTiles("Stamen.Toner") %>% 
  addPolygons(data = df_sf_buffer_1000,
              fillOpacity = 0.2, 
              group = "Zone tampon (1000m)", color = 'red')%>%
  addPolygons(data = df_sf_buffer_5000,
              fillOpacity = 0.2, 
              group = "Zone tampon (5000m)", color = 'red')%>%
  addCircles(data = transports,
             color = "blue",
             fill = "blue",
             opacity = 1, 
  ...
  # Layers control
  addLayersControl(
    overlayGroups = c("Ma position", "Zone tampon (1000m)", "Zone tampon (5000m)","Station de bus"),
    options = layersControlOptions(collapsed = FALSE)
  )  %>%
  addLegendCustom(colors = c("red","yellow","blue"), labels = c("Zone tampon","Ma position", "Station de bus"), sizes = c(20,20))
...
map <- leaflet() %>%
  ...
  addCircles(data = station_nextToMe,
             color = "blue",
             fill = "blue",
             opacity = 1, 
             group = "Station  à proximité") %>%
  addCircles(data = df_sf,
             color = "yellow",
             fill = "yellow",
             opacity = 1, 
  ...
  # Layers control
  addLayersControl(
    overlayGroups = c("Ma position", "Station  à proximité", "Zone tampon"),
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  addLegendCustom(colors = c("red","yellow","blue"), labels = c("Zone tampon","Ma position", "Station de bus"), sizes = c(20,20))
...
# Distance ----------------------------------------------------------------
distances.mx <- as.numeric((st_distance(df_sf,station_nextToMe)))
distances.df <- data.frame(place=rep("Station de bus", length(distances.mx)),distance=distances.mx)
station_nextToMe$distance <- distances.df$distance
...
box.plot_distance <- ggplot(data = distances.df, aes(x=place, y=distance)) + 
  ...
  labs(title="Distances entre ma position et les stations de bus",
...
curveBoxplot.plot_distance <- ggarrange(box.plot_distance, curve.plot_distance, ncol = 2, nrow = 1)

results[['curveBoxplot.plot_distance']] <- curveBoxplot.plot_distance


# temps de parcours  -------------------------------------------------------------------------

routeToMin <- osrmRoute(src = df_sf, dst = station_nextToMe[which.min(station_nextToMe$distance),], returnclass = "sf", osrm.profile = "foot")
routeToMin_coords <- as.data.frame(st_coordinates(routeToMin))
routeToMax <- osrmRoute(src = df_sf, dst = station_nextToMe[which.max(station_nextToMe$distance),], returnclass = "sf", osrm.profile = "foot")
routeToMax_coords <- as.data.frame(st_coordinates(routeToMax))

map <- leaflet() %>%
  addProviderTiles("Stamen.Toner") %>% 
  addCircles(data = station_nextToMe[which.max(station_nextToMe$distance),],
             color = "blue",
             fill = "blue",
             opacity = 1, 
  ...
  addCircles(data = station_nextToMe[which.min(station_nextToMe$distance),],
             color = "red",
             fill = "red",
             opacity = 1, 
